...
// Companion snapshot files that commonly change when registry-backed extension commands
// are added or updated. Generated by TestFigSpec (see cli/azd/docs/fig-spec.md). In-place
// modifications of these files may accompany simple registry updates without requiring
// core review; deletions and renames still require core review.
const ALLOWED_NON_REGISTRY_PATHS = new Set([
...
function isAllowedCompanionFileChange(file) {
  if (!ALLOWED_NON_REGISTRY_PATHS.has(file.filename) || file.previous_filename != null) {
    return false;
  }

  // GitHub marks in-place content edits as 'modified'. Tests may omit status.
  return file.status == null || file.status === 'modified';
}

/**
 * Flags file-level changes that disqualify a PR from the registry-only fast path.
 * Only in-place edits to the known registry files (plus in-place edits of a small
 * allowlist of companion snapshot files) may skip core review; touching any other
 * file, or renaming/deleting a registry or companion file, requires a core reviewer.
 *
...
function diffChangedFiles(changedFiles) {
  ...
  const nonRegistryFiles = changedFiles
    .filter((file) => !isRegistryFile(file) && !isAllowedCompanionFileChange(file))
