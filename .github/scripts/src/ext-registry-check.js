...
// are added or updated. Generated by TestFigSpec (see cli/azd/docs/fig-spec.md). These may
// accompany simple registry updates without requiring core review.
 ...
 * Only in-place edits to the known registry files (plus a small allowlist of companion
 * snapshot files) may skip core review; touching any other file, or renaming/deleting a
 * registry file, requires a core reviewer.
 *
...
function diffChangedFiles(changedFiles) {
  ...
  const nonRegistryFiles = changedFiles
    .filter((file) => !isRegistryFile(file) && !ALLOWED_NON_REGISTRY_PATHS.has(file.filename))
