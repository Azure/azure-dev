...
description: >-
  **WORKFLOW SKILL** — Runs `go tool mage preflight` from `cli/azd/` and auto-fixes failures.
  Covers linting, formatting, copyright, spelling, build, and tests.
  Iterates fix-then-rerun cycles until all checks pass.

  INVOKES: go CLI, golangci-lint, cspell, gh CLI, bash/sh, ask_user.
...
The azd preflight suite (`go tool mage preflight`) validates code quality before
...
- `go tool mage preflight` exits successfully, or unresolved failures are clearly reported
