...
2. Verify tools: `go`, `golangci-lint`, `cspell`, `gh`.
...
```bash
# If at repo root:
cd cli/azd && go tool mage preflight
...
Use the command's exit status as the source of truth.

**If the command exits successfully** and every changed changelog is clean: Report success and stop. No fixes needed.

**If the command exits nonzero**: Use the summary and captured output to identify failed or
skipped checks, then proceed to Step 4 for each failure. A skipped check does not make the
preflight successful.

Also proceed to Step 4 for each targeted changelog spell-check failure.

### Step 4 — Fix Failures (Iterative)

For each failing check, apply the fix strategy from the references. Process checks in
their reported order because earlier fixes can resolve later failures (e.g., `gofmt`
fixes may resolve `lint` issues, `build` fixes resolve `test` failures).

After the standard checks, fix any targeted changelog spelling failures.
...
- All checks and every changed changelog pass, OR
...
with full context and ask via `ask_user` whether to continue with a guided fix or stop with
