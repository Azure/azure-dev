...
- **Core**: use the version specified in the triggering issue or user request if explicit (e.g., "Create changelog for 1.24.2"); otherwise derive from the existing unreleased header (strip `-beta.*` and `(Unreleased)`), use today's date. Replace that top unreleased header with the dated release entry; do not add a new unreleased placeholder above it. Update `cli/version.txt` and `cli/azd/pkg/azdext/version.go` (`Version` constant) to the released version.
