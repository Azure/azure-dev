...
| WSL setup tools (Windows only) | Inside WSL, verify `git`, `curl` or `wget`, `awk`, `grep`, `tar`, `sha256sum`, `sha512sum`, `uname`, and `sudo` are available | **Hard stop.** `setup-wsl.sh` bootstraps the exact Go version from `cli/azd/go.mod`, .NET SDK from `dotnet-sdk.version`, native uv from `uv.version`, and Python 3.13+ when needed, but it does not install general OS packages. Tell the user which setup tool is missing. |
| Native Linux test runtimes in WSL (Windows only) | After workflow Step 2, verify `/usr/local/bin/azd`, the expected dev version, the pinned .NET SDK, native `uv`, and Python 3.13+ | **Hard stop.** Re-run workflow Step 2 (`setup-wsl.sh`). It reuses compatible uv and Python installations and installs only missing runtimes. Do not proceed until every check passes. |
| Native Linux test runtimes (native Linux/macOS) | `azd version` returns the expected dev build, `uv --version` succeeds, and `python3 --version` is 3.13+ | Ask the user to build/install the missing runtime. No special path check is needed for native `azd`. |
