...
func mapError(err error, span tracing.Span) {
	...
	var armDeployErr *azureapis.AzureDeploymentError
	...
	if errors.As(err, &respErr) {
		...
		var collect func(details []*azureapis.DeploymentErrorLine, frame int)
		collect = func(details []*azureapis.DeploymentErrorLine, frame int) {
		...
		collect([]*azureapis.DeploymentErrorLine{armDeployErr.Details}, 0)
		if len(codes) > 0 {
...
func collectCode(lines []*azureapis.DeploymentErrorLine, frame int) *deploymentErrorCode {
	if len(lines) == 0 {
		return nil
	}
