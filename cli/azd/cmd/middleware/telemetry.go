...
func mapError(err error, span tracing.Span) {
	...
	var armDeployErr *deploymentservice.AzureDeploymentError
	...
	if errors.As(err, &respErr) {
		...
		var collect func(details []*deploymentservice.DeploymentErrorLine, frame int)
		collect = func(details []*deploymentservice.DeploymentErrorLine, frame int) {
		...
		collect([]*deploymentservice.DeploymentErrorLine{armDeployErr.Details}, 0)
		if len(codes) > 0 {
...
func collectCode(lines []*deploymentservice.DeploymentErrorLine, frame int) *deploymentErrorCode {
	if len(lines) == 0 {
		return nil
	}
