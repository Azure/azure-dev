...
func upCmd(rootOptions *commands.GlobalCommandOptions) *cobra.Command {
	cmd := commands.Build(
		commands.CompositeAction(
			...
			commands.ActionFunc(
				func(ctx context.Context, cmd *cobra.Command, args []string, azdCtx *azd_context.AzdContext) error {
...
func (a *ignoreInitErrorAction) Run(ctx context.Context, cmd *cobra.Command, args []string, azdCtx *azd_context.AzdContext) error {
