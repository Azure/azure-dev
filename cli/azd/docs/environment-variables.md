...
| `AZD_ALLOW_NON_EMPTY_FOLDER` | If true, allows `azd init` to run in a non-empty directory without prompting. |
| `AZD_NO_PROMPT` | If true, disables all interactive prompts. |
| `AZD_ENVIRONMENT` | Specifies the default environment name for azd commands. |
| `AZD_CWD` | Overrides the current working directory for azd commands. |
| `AZD_SERVER` | If true, starts `azd` in server mode. |
| `AZD_ACCESS_TOKEN` | An access token used for authenticating API requests in server mode. |
...
| Variable | Description |
| --- | --- |
| `AZD_EXT_TIMEOUT` | Timeout duration for extension operations. |
| `AZD_EXT_DEBUG` | If true, enables debug output for extensions. |
| `AZD_EXTENSION_CACHE_TTL` | The time-to-live for extension cache entries. |
