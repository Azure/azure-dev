...
| `AZD_SERVER` | The address (e.g., `localhost:12345`) of the azd extension server for gRPC communication. Set by the extension framework. |
| `AZD_ACCESS_TOKEN` | A JWT used by the extension host to authenticate gRPC calls to the azd extension server. Generated by the extension framework. |
...
| `AZD_AUTH_CERT` | The [External Authentication](./external-authentication.md) client certificate, provided as a base64-encoded DER X.509 certificate string. When set, `AZD_AUTH_ENDPOINT` must use HTTPS. |
...
| Variable | Description |
| --- | --- |
| `AZD_EXT_TIMEOUT` | Timeout for extension operations, in integer seconds (e.g., `10`). Defaults to 5 seconds. |
| `AZD_EXT_DEBUG` | If true, enables debug output for extensions. |
| `AZD_EXTENSION_CACHE_TTL` | Time-to-live for extension cache entries, as a Go duration string (e.g., `30m`, `4h`). Defaults to `4h`. |
