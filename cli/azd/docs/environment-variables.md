...
| `AZD_DEPLOYMENT_ID_FILE` | Absolute path of a file that `azd` writes (overwriting any existing file) with a JSON document containing the ARM deployment ID as soon as it is available during `azd provision` or `azd up`. The containing directory must already exist and be writable. The current document shape is `{ "deploymentId": "/subscriptions/.../providers/Microsoft.Resources/deployments/<name>" }`; consumers must ignore unknown fields so the format can be extended. If the file cannot be written (for example, the directory does not exist or is not writable) the error is logged and provisioning continues. Only Bicep deployments are supported. |
