...
| `AZD_DEPLOYMENT_ID_FILE` | Absolute path of a file that `azd` writes (overwriting any existing file) with a JSON document containing the ARM deployment ID just before the ARM deployment is submitted during `azd provision` or `azd up`. The path must be absolute (relative paths are ignored); the containing directory must already exist and be writable. The file is only written when an ARM deployment is actually started — runs short-circuited by the deployment-state cache or aborted by preflight do not produce a file. The current document shape is `{ "deploymentId": "/subscriptions/.../providers/Microsoft.Resources/deployments/<name>" }`; consumers must ignore unknown fields so the format can be extended. The file is written atomically (temp file + rename) so concurrent readers always see a complete document. If the file cannot be written (for example, the parent directory does not exist, the path is not writable, or the path points to a directory rather than a file), provisioning continues and the failure is recorded via the standard log; that output is only visible when `--debug` or `AZD_DEBUG_LOG` is enabled. On Windows, if the consuming tool holds a read handle on the file when a new layer writes, `os.Rename` will fail — consumers should use a file-watcher pattern that does not keep the file open. Only Bicep deployments are supported. |
