...
| `AZD_NON_INTERACTIVE` | Controls no-prompt mode. Parsed as a boolean (`true`/`false`/`1`/`0`). Set to a truthy value to run without interactive prompts (equivalent to `--no-prompt`). `azd` also auto-enables no-prompt mode when it detects a CI/CD or AI-agent environment; setting this variable (to **any** value, including `false`) opts out of that automatic enablement, so `AZD_NON_INTERACTIVE=false` keeps the global no-prompt setting off in those environments. Note that some commands still avoid interactive prompts in CI/CD by design, independent of this variable. Explicit `--no-prompt`/`--non-interactive` flags take precedence over this variable. |
