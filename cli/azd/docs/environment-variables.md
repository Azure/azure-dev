...
| `AZD_CONTAINER_RUNTIME` | Selects `docker` or `podman`. If unset or empty, azd prefers Docker on PATH, then Podman. Selection is cached on first use for each container CLI instance; version and daemon readiness checks are repeated when needed. Other values fail the runtime installation check. The [RLE extension](../extensions/azure.ai.rle/README.md) also reads this variable to select its container executable, but defaults to `docker` without automatic fallback. |
