...
A PR check scans telemetry payload construction in production Go source and
checks every attribute against that inventory before the extension is
released. This keeps classification failures in development and out of the
user's command path.
...
`FeatureInsight` merely because another extension field uses them. Customer
content is rejected and must not be sent through telemetry. Use classification,
purpose, and endpoint values already established by core `azd` fields.
...
compile-time string constants so the source validator can resolve them.
...
release, while released extensions continue to use the existing best-effort
`ReportUsage` behavior.
