...
Primary list-command output — such as `azd tool list`, `azd tool check`, `azd extension list` (and its alias `azd ext list`), `azd extension source list`, `azd template list`, `azd template source list`, and `azd copilot consent list` — renders through the shared responsive formatter. This guidance does not apply to detail screens or operation previews.
...
| ----------- | ----------------------------------------- | ------------ |
| **Full**    | `width >= 100` (`DefaultFullThreshold`)    | All columns; long values may still wrap or truncate to fit |
...
- Set `ForceCards: true` only when a command should always use the stacked-row layout regardless of width.
...
columns := []output.PrettyColumn{
    ...
    },
    {
        ...
        Priority:    1,
        ColorFunc:   statusColor, // command-specific semantic status helper
    },
    {Column: output.Column{Heading: "SOURCE", ValueTemplate: "{{.Source}}"}, Priority: 3},
}
...
| `Incompatible`      | Latest update is incompatible with the current azd version | `WithErrorFormat` |
...
- **Empty state**: short-circuit before calling `Format` and write a clear, single-line message through `console.Message`. Use `WithGrayFormat` for a neutral empty result; use warning styling only when the state requires user action. Add concise recovery guidance when appropriate.
- **Stable ordering**: the formatter does not sort — it renders and groups stacked rows in the order they are passed in. **Sort your rows before formatting** so output is deterministic.
