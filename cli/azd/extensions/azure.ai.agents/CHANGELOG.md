...
- [[#8584]](https://github.com/Azure/azure-dev/issues/8584) Fix `azd ai agent` commands hanging silently in non-TTY contexts (CI, agent harnesses, or output piped through `tee`/`tail`) when the target agent is ambiguous. The interactive "Select an agent service" picker now detects when interactive prompting is unavailable and fails fast with an actionable error (listing the available services and how to pass one) instead of blocking forever on stdin. In interactive mode it also prints a one-line banner to stderr before the picker so the user can see why azd is waiting.
