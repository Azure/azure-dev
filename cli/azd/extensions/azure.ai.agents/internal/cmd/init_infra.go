...
func ejectTerraform(projectRoot, infraDir string, params map[string]any) error {
	written, err := writeEmbeddedTerraformTemplates(infraDir)
	if err != nil {
		return err
	}
...
// writeEmbeddedTerraformTemplates copies every *.tf file under the embedded
// templates/terraform/ root into infraDir (flat -- the module has no
// submodules) and returns the files written. On any error it removes the
// partial infraDir.
//
// main.tfvars.json is intentionally NOT embedded: it is generated by
// writeTfvarsFile from the synthesized parameters, so it never goes stale.
func writeEmbeddedTerraformTemplates(infraDir string) (_ []ejectArtifact, retErr error) {
	//nolint:gosec // G301: ejected infra/ directory must be readable/traversable by IDEs, Git, and CI
	if err := os.MkdirAll(infraDir, 0o755); err != nil {
		return nil, exterrors.Internal(
			exterrors.CodeInfraEjectWriteFailed,
...
// include_acr) are written literally; deploy-time inputs (location,
...
func writeTfvarsFile(infraDir string, params map[string]any) (ejectArtifact, error) {
	...
	// Map synthesizer parameter names to Terraform variable names. The Bicep
	// param `includeAcr` is `include_acr` in HCL; `deployments` is unchanged.
	if v, ok := params["includeAcr"]; ok {
		doc["include_acr"] = v
	}
