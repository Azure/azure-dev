...
func commitTeamsAppPackage(packagePath, markerPath string, zipBytes []byte) (string, error) {
	...
	if packageExisted && !teamsAppPackageIsOwned(markerPath) {
		log.Printf(
	...
	if err := writeTeamsAppFileAtomically(markerPath, []byte("generated by azd\n")); err != nil {
		if !packageExisted {
			if removeErr := os.Remove(packagePath); removeErr != nil && !errors.Is(removeErr, os.ErrNotExist) {
				log.Printf("postdeploy: could not roll back Teams app package %q after marker write failed: %v", packagePath, removeErr)
			}
		}
