...
func TestPersistPromptAgentCandidateConfigPreservesRawReferences(t *testing.T) {
	...
	for _, legacy := range []bool{false, true} {
		for _, fileName := range []string{"azure.yaml", "azure.yml"} {
			t.Run(fmt.Sprintf("legacy=%t/%s", legacy, fileName), func(t *testing.T) {
				t.Parallel()
...
func TestPersistPromptAgentCandidateConfigRejectsInvalidProjectShape(t *testing.T) {
	...
	tests := []struct {
		...
		{"missing legacy config", "services:\n  prompt-agent:\n    host: azure.ai.agent", true,
			"is missing or not a mapping"},
		{"legacy config scalar", "services:\n  prompt-agent:\n    config: invalid", true,
			"is missing or not a mapping"},
		{"legacy config list", "services:\n  prompt-agent:\n    config: []", true, "is missing or not a mapping"},
	}
