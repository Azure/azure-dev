...
// recordPendingConnectionProvision stores a connection pending-provision
// reason after init actually writes at least one connection service.
// Write failures are warnings so a successful init is not rolled back.
