...
func writeTeamsSideloadScripts(
	...
	for _, s := range scripts {
		...
		if info, statErr := os.Lstat(scriptPath); statErr == nil {
			...
			if existing, readErr := os.ReadFile(scriptPath); readErr != nil ||
				!strings.Contains(string(existing), teamsSideloadGeneratedMarker) {
				log.Printf(
					"postdeploy: %q already exists and was not generated by azd; "+
						"leaving it untouched (rename it to get the Teams sideload script)",
					scriptPath,
				)
				continue
			}
