...
func TestTerraformTemplatesFS_Embedded(t *testing.T) {
	...
	// main.tfvars.json is generated by the eject step, not embedded, so it must
	// NOT be present in the embedded FS (otherwise it would go stale).
	_, err := fs.ReadFile("templates/terraform/main.tfvars.json")
	assert.Error(t, err, "main.tfvars.json must not be embedded; it is generated at eject time")
}

// TestTerraformModule_DerivesProjectNameWhenEmpty guards the regression where an
// unset AZURE_AI_PROJECT_NAME substituted to "" in main.tfvars.json and failed
// the foundry_project_name validation. The fix: variables.tf must allow an empty
// value, and main.tf must derive the name from environment_name. This asserts the
// embedded templates still carry that fallback so it cannot silently regress.
func TestTerraformModule_DerivesProjectNameWhenEmpty(t *testing.T) {
	fs := TerraformTemplatesFS()
