...
`)
		...
		res, err := Synthesize(Input{RawAzureYAML: raw, ServiceName: "project", Env: map[string]string{
			"SEARCH_KEY": "secret",
		}, ProjectRoot: root})
		...
		assert.Equal(t, "secret", res.ConnectionCredentials["search"]["key"])
	})
}
...
`)
	...
	res, err := Synthesize(Input{RawAzureYAML: raw, ServiceName: "my-project", ProjectRoot: root})
...
`)
	...
	require.NoError(t, err)
	assert.False(t, result.RequiresAcr)
}
...
`)
	...
	require.NoError(t, err)
	assert.True(t, result.RequiresAcr)
}
