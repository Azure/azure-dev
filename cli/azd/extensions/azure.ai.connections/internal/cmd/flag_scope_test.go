	...
	for _, name := range []string{"context", "version", "metadata", "listen"} {
		for _, flag := range []string{"--project-endpoint=", "--project-endpoint=https://example.test", "-p="} {
			t.Run(name+flag, func(t *testing.T) {
				...
				require.Contains(t, local.Message, "connection "+name)
...
func TestRemoteFlagScopePreservesCommands(t *testing.T) {
	t.Setenv("AZD_CWD", "")
	t.Setenv("AZD_DEBUG", "false")
	for _, name := range []string{"list", "show", "create", "update", "delete", "context", "version", "metadata"} {
		t.Run(name, func(t *testing.T) {
			root := NewRootCommand()
			command, _, err := root.Find([]string{name})
			require.NoError(t, err)
			// Replace only the action and argument requirements; preserve parsing and all pre-runs.
			command.Args = nil
