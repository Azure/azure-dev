...
func TestDownloadContainerBackedSingleFileWritesExactDestination(t *testing.T) {
	...
	require.NoError(t, action.downloadWith(t.Context(), &datasetContext{datasetClient: client}))
	got, err := os.ReadFile(dest) //nolint:gosec // this test's own temp dir
...
func TestDownloadContainerBackedFileDestinationsAndOverwrite(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			err := action.downloadWith(t.Context(), &datasetContext{datasetClient: client})
...
func TestDownloadFolderPreservesEveryFile(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			ec := &datasetContext{datasetClient: client}
			...
			require.NoError(t, action.cmd.Flags().Set("output", outputTable))
			require.NoError(t, action.downloadWith(t.Context(), ec))
			for name, want := range tc.files {
				//nolint:gosec // this test's own temp dir
