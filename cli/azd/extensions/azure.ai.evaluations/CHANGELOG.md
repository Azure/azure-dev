...
- `generate` synthesizes a rubric and a dataset from the agent's context, writes
  ...
  preserving comments, ordering and neighbouring entries.
...
- Every listing is pageable rather than walking the whole history: `--limit`
  bounds a page, `--after` continues from the last one, and `--all` asks for
