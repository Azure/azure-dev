...
func TestScanJSONL(t *testing.T) {
	...
	all, err := scanJSONL(bytes.NewReader(content), 0)
...
func TestScanJSONL_StopsAtTheLimit(t *testing.T) {
	...
	two, err := scanJSONL(bytes.NewReader(content), 2)
	require.NoError(t, err)
	require.Len(t, two, 2)
	...
	more, err := scanJSONL(bytes.NewReader(content), 99)
	require.NoError(t, err)
	assert.Len(t, more, 3)
}

func TestScanJSONL_ReportsTheOffendingLine(t *testing.T) {
	_, err := scanJSONL(bytes.NewReader([]byte("{\"n\":1}\nnot json\n")), 0)
	require.ErrorContains(t, err, "line 2")
}

func TestScanJSONL_EmptyIsNotAnError(t *testing.T) {
	items, err := scanJSONL(bytes.NewReader([]byte("\n\n")), 0)
	require.NoError(t, err)
