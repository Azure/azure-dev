...
func TestResolveTraceWindow_LookbackMeasuresBackFromTheEnd(t *testing.T) {
	start, end, err := ResolveTraceWindow(&SourceDecl{
...
func TestResolveTraceWindow_LookbackWithNoEndMeasuresBackFromNow(t *testing.T) {
	start, end, err := ResolveTraceWindow(&SourceDecl{
...
func TestResolveTraceWindow_OpenWindowIsFine(t *testing.T) {
	start, end, err := ResolveTraceWindow(&SourceDecl{Type: SourceTypeTraces, AgentName: "a"})

	require.NoError(t, err)
	assert.True(t, start.IsZero())
	assert.True(t, end.IsZero())

	start, end, err = ResolveTraceWindow(nil)
	require.NoError(t, err)
	assert.True(t, start.IsZero())
	assert.True(t, end.IsZero())
}
...
func TestResolveTraceWindow_Refuses(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			_, _, err := ResolveTraceWindow(&tc.source)
			require.Error(t, err)
...
// The bound is exactly on the line, so the check is `>` and not `>=`.
func TestResolveTraceWindow_AcceptsTheLargestLookbackAllowed(t *testing.T) {
