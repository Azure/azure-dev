...
func TestAIHelp(t *testing.T) {
	...
	walkAIHelp(t, func(t *testing.T, command *cobra.Command, path []string) {
		...
		if len(path) == 0 {
			require.Contains(t, text, "\nLocal Agent Context\n")
			require.Contains(t, text, "localhost:8088")
			require.NotContains(t, text, ".azure/<environment>/.env")
		} else {
			require.NotContains(t, text, "\nLocal Agent Context\n")
		}
...
func TestAIHelpColor(t *testing.T) {
	...
	walkAIHelp(t, func(t *testing.T, _ *cobra.Command, path []string) {
		...
		if len(path) == 0 {
			require.Contains(t, text, output.WithBold("%s", output.WithUnderline("Local Agent Context")))
		}
