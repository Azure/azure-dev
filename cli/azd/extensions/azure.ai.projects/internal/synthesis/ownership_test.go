...
func TestSynthesisValidatesToolboxAfterFileReference(t *testing.T) {
	t.Parallel()
	for _, test := range []struct {
		name, payload string
		wantError     bool
	}{
		{name: "reference", payload: "name: tools\n"},
		{name: "bundled definition", payload: "name: tools\ntools: []\n", wantError: true},
	} {
		t.Run(test.name, func(t *testing.T) {
			t.Parallel()
			root := t.TempDir()
			require.NoError(t, os.WriteFile(filepath.Join(root, "toolbox.yaml"), []byte(test.payload), 0o600))
			result, err := Synthesize(Input{
				ServiceName: "project", ProjectRoot: root,
				RawAzureYAML: []byte("services:\n  project:\n    host: azure.ai.project\n" +
					"  agent:\n    host: azure.ai.agent\n    kind: hosted\n    toolboxes:\n      - $ref: ./toolbox.yaml\n"),
			})
			if test.wantError {
				require.ErrorContains(t, err, "bundled declarations")
				assert.Nil(t, result)
			} else {
				require.NoError(t, err)
				assert.NotContains(t, result.Parameters, "connections")
