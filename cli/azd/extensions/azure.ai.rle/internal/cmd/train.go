...
environment supplies the reward signal instead of a grader, so no training file is
required. rl_environment is currently hidden from finetunesapi's public API surface and
only completes for base models enabled for Loom-backed RL-environment training. Job
creation fails if the base model is not enabled, or if the RLE version is not published
and ready in the project set by FOUNDRY_PROJECT_ENDPOINT.`,
		Args: cobra.NoArgs,
	...
	cmd.Flags().StringVar(&flags.trainingFile, "training-file", "",
		"File id of an uploaded training dataset. Not required for RLE-backed training.")
	...
	for _, name := range []string{"rle-name", "rle-version", "model"} {
