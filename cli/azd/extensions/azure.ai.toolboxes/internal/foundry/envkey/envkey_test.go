...
// TestToolboxMCPEndpoint mirrors the cases in the sibling azure.ai.agents
// envkey test so the duplicated key builder cannot drift from the one the
...
func TestToolboxMCPEndpoint(t *testing.T) {
	...
	tests := []struct {
		...
		// Run-collapsing - without it the agent doctor would search for
		// TOOLBOX_MY__TOOL_MCP_ENDPOINT and miss the real value.
		{"double-hyphen-run", "my--tool", "TOOLBOX_MY_TOOL_MCP_ENDPOINT"},
		// Symbol classes that a rune-by-rune normalizer would miss.
		...
		// Trailing non-alphanum produces a trailing underscore inside the
		// sanitized segment, consistent with the agents extension.
