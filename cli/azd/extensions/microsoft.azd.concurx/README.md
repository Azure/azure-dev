...
```bash
azd extension add microsoft.azd.concurx
```

*If installing from a local source during development:*

```bash
azd extension add <path-to-extension-folder>
```
...
2. Once profiled, launch `azd deploy <service-name>` for all services defined in your `azure.yaml` concurrently.
