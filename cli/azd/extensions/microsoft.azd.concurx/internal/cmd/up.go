...
func runUpCommand(cmd *cobra.Command, args []string) error {
	...
	defer promptServer.Stop()

	// Create Bubble Tea UI with cancel function and prompt server
	ui := createUI(services, cancel, promptServer)

	// Set the UI on the prompt server so it can send messages
	promptServer.SetUI(ui)
	promptServer.Start()

	// Create and run concurrent deployer
	deployer, err := NewConcurrentDeployer(ctx, workflowClient, services, ui, debug, promptServer)
	if err != nil {
		return err
	}
...
func initializeAzdClient(ctx context.Context) (
	...
	if err := azdext.WaitForDebugger(ctx, azdClient); err != nil {
		...
		return nil, nil, nil, fmt.Errorf("failed waiting for debugger: %w", err)
	}
...
func createUI(services map[string]*azdext.ServiceConfig, cancel context.CancelFunc, promptServer *PromptServer) *tea.Program {
	serviceNames := make([]string, 0, len(services))
	for name := range services {
		serviceNames = append(serviceNames, name)
	}

	model := newDeploymentModel(serviceNames, cancel, promptServer)
