...
// Package executor provides secure script execution with Azure context.
package executor
...
func (c *Config) Validate() error {
	if err := shellutil.ValidateShell(c.Shell); err != nil {
...
func (e *Executor) Execute(ctx context.Context, scriptPath string) error {
	...
	if shell == "" {
		shell = shellutil.DetectShellFromFile(absPath)
	}
...
// (e.g. `azd exec python script.py --port 8000`). Stdin is always connected
// because the caller is launching a process, not evaluating a shell expression.
func (e *Executor) ExecuteDirect(ctx context.Context, command string, args []string) error {
	...
	cmd := exec.CommandContext(ctx, command, args...) //nolint:gosec // CLI exec; command comes from user argv
	cmd.Dir = workingDir
	cmd.Env = os.Environ()
	cmd.Stdin = os.Stdin
	...
	if os.Getenv("AZD_DEBUG") == "true" {
		...
		fmt.Fprintf(os.Stderr, "Executing (direct): %s %s\n", command, strings.Join(quotedArgs, " "))
