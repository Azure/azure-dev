...
class ContextCommand:
    ...
    async def execute(self):
        try:
            # User Config
            config_response = self.azd_client.user_config.Get(user_config_pb2.GetUserConfigRequest())
            ...
            # Project
            project_response = self.azd_client.project.Get(empty_pb2.Empty())
            if project_response:
                print("Project:")
            ...
            env_list = self.azd_client.environment.List(empty_pb2.Empty())
            ...
            # Environment values
            env_values = self.azd_client.environment.GetValues(
            ...
            # Deployment Context
            deployment_ctx = self.azd_client.deployment.GetDeploymentContext(empty_pb2.Empty())
            
            if deployment_ctx:
                scope = deployment_ctx.AzureContext.scope
                scope_map = {
                ...
                for resource_id in deployment_ctx.AzureContext.resources:
