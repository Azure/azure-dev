...
func NewServer(
	...
	if provider, ok := eventService.(interface{ betaEventServiceOverride() any }); ok {
		server.WithOptions(WithBetaServiceOverride(BetaEventService, provider.betaEventServiceOverride()))
	}
	return server
}
