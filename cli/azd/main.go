...
func fetchLatestVersion(version chan<- semver.Version) {
	...
	// of time
	azdConfigDir, err := config.GetUserConfigDir()
	if err != nil {
		log.Printf("could not get user config directory: %v, skipping update check", err)
		return
	}

	cacheFilePath := filepath.Join(azdConfigDir, updateCheckCacheFileName)
