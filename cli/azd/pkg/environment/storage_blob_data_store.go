...
func NewStorageBlobDataStore(blobClient storage.BlobClient) RemoteDataStore {
...
func (sbd *StorageBlobDataStore) Save(ctx context.Context, env *Environment) error {
	...
	if err := cfgMgr.SaveTo(env.Config, cfgWriter); err != nil {
...
func (sbd *StorageBlobDataStore) Reload(ctx context.Context, env *Environment) error {
	...
	cfgMgr := config.NewManager()
	if cfg, err := cfgMgr.LoadFrom(configBuffer); errors.Is(err, os.ErrNotExist) {
