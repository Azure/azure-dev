...
func TestValidateRegistry_DuplicateLatestServiceTargetProvider(t *testing.T) {
	registry := &Registry{
		SchemaVersion: "1.0.0",
		Extensions: []*ExtensionMetadata{
			{
				...
				Versions: []ExtensionVersion{
					{
						...
						Providers: []Provider{
							{Name: "azure.ai.project", Type: ServiceTargetProviderType},
						},
						Artifacts: validArtifacts(),
					},
				},
...
func TestValidateRegistry_DuplicateServiceTargetProviderInLatestVersion(t *testing.T) {
	registry := &Registry{
		SchemaVersion: "1.0.0",
		Extensions: []*ExtensionMetadata{
			{
	...
	require.Contains(t,
		issueMessages(result.Issues),
		`extension "publisher.extension" declares service-target provider "Azure.AI.Project" `+
			`more than once in latest version "1.0.0"`,
	)
}
