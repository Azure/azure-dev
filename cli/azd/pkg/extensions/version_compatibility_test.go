...
func Test_VersionIsCompatible(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			minAzdVersion:  "99.0.0",
			azdVersion:     "0.0.0-dev.0",
			expectCompatible: true,
		},
		{
			name:           "daily build always compatible",
			minAzdVersion:  "99.0.0",
			azdVersion:     "0.0.0-daily.12345",
			expectCompatible: true,
		},
		{
...
func Test_FilterCompatibleVersions(t *testing.T) {
	...
	t.Run("dev build all compatible", func(t *testing.T) {
		versions := []ExtensionVersion{
			{Version: "1.0.0", MinAzdVersion: "99.0.0"},
			{Version: "2.0.0", MinAzdVersion: "99.0.0"},
		}
		azdVersion := semver.MustParse("0.0.0-dev.0")
		result := FilterCompatibleVersions(versions, azdVersion)
