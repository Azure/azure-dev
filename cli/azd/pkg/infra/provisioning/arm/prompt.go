...
func (p *ArmProvider) promptForParameter(
	ctx context.Context,
	key string,
	...
	paramType := p.mapArmTypeToInterfaceType(param.Type)
	...
	if paramType == ParameterTypeString && azdMetadata.Type != nil && *azdMetadata.Type == "location" {
		...
		case ParameterTypeObject:
			...
			value = userValue
		default:
			panic(fmt.Sprintf("unknown parameter type: %s", p.mapArmTypeToInterfaceType(param.Type)))
		}
	}
