...
func (p *BicepProvider) EnsureEnv(ctx context.Context) error {
	...
	// a virtual environment that also collects outputs generated by previous modules
	lookupEnv := func(name string) (string, bool) {
		val, inEnv := p.env.LookupEnv(name)
		if inEnv {
			return val, true
		}
		...
		_, ok := outputMap[name]
		if ok {
			// any non-empty value will skip prompting
			return fmt.Sprintf("${%s}", name), ok
		}
	...
	for _, modulePath := range p.modulePaths() {
		...
		if isBicepFile(modulePath) {
			...
			_, err = p.ensureParameters(ctx, compileResult.Template, modulePath, lookupEnv)
			if err != nil {
				return err
			}
		}
...
func (p *BicepProvider) plan(ctx context.Context, modulePath string) (*deploymentDetails, error) {
	...
	if isBicepFile(modulePath) {
		configuredParameters, err := p.ensureParameters(ctx, compileResult.Template, modulePath, nil)
		if err != nil {
			return nil, err
		}
...
// moduleDisplayNameFromPaths returns the module display name from the provided paths.
//
// For example: infra, infra/app
func moduleDisplayNameFromPaths(modulePath string, rootModulePath string) string {
	rootModuleDir := filepath.Dir(rootModulePath)
	moduleDir := filepath.Dir(modulePath)
	if !strings.HasPrefix(moduleDir, rootModuleDir) {
		panic(fmt.Sprintf("moduleDir %s does not start with rootModuleDir %s", moduleDir, rootModuleDir))
...
func (p *BicepProvider) Deploy(ctx context.Context) (*provisioning.DeployResult, error) {
	...
	progressDisplay := func(moduleName string, deployment infra.Deployment) chan<- bool {
		...
		if use, err := strconv.ParseBool(os.Getenv("AZD_DEBUG_PROVISION_PROGRESS_DISABLE")); err == nil && use {
			...
			cancelProgress = make(chan bool, 1)
		} else {
			go func() {
				...
				pd := p.deploymentManager.ProgressDisplay(deployment)
	...
	captureOutputs := func(outputs map[string]provisioning.OutputParameter) error {
		for key, value := range outputs {
			result.Deployment.Outputs[key] = value
		}

		return provisioning.UpdateEnvironment(ctx, p.envManager, p.env, outputs)
	}

	allSkipped := true
	for i, modulePath := range modulePaths {
		moduleName := moduleNameFromPaths(modulePath, rootModulePath)
		moduleDisplayName := moduleDisplayNameFromPaths(modulePath, rootModulePath)

		p.console.EnsureBlankLine(ctx)

		displayTitle := ""
		if len(modulePaths) > 1 {
			displayTitle = fmt.Sprintf("  Deployment %d of %d - %s\n", i+1, len(modulePaths), moduleDisplayName)
			p.console.Message(ctx, displayTitle)
		}
		...
		if !p.ignoreDeploymentState && parametersHashErr == nil {
			...
			if err == nil {
				...
				if err := captureOutputs(deployment.Outputs); err != nil {
					return nil, err
				}

				// for the last deployment, the message is shown in provisioning/manager.go currently
				if len(modulePaths) > 1 && i < len(modulePaths)-1 {
					p.console.StopSpinner(ctx, "Didn't find new changes.\n", input.StepSkipped)
				}
				continue
			}
			logDS("%s", err.Error())
		...
		cancel := progressDisplay("", bicepDeploymentData.Target)
		...
		err = captureOutputs(deployment.Outputs)
		if err != nil {
			return nil, err
		}
	}
...
func (p *BicepProvider) loadParameters(
	ctx context.Context,
	modulePath string,
	lookupEnv func(string) (string, bool)) (loadParametersResult, error) {
	if lookupEnv == nil {
		lookupEnv = p.env.LookupEnv
	}
	...
	for paramName, param := range decodedParamsFile.Parameters {
		...
		replaced, err := envsubst.Eval(string(paramBytes), func(name string) string {
			...
			val, isDefined := lookupEnv(name)
			if !isDefined {
				// if any env var is not set, we track the parameter as unresolved
				hasUnsetEnvVar = true
			}
...
func (p *BicepProvider) azdModulePath() string {
	...
	moduleDir := filepath.Join(infraRoot, "app")
...
func (p *BicepProvider) ensureParameters(
	...
	parametersResult, err := p.loadParameters(ctx, modulePath, lookupEnv)
	if err != nil {
