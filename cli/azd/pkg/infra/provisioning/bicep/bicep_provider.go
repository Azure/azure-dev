...
func (p *BicepProvider) prevDeploymentEqualToCurrent(
	...
	if err != nil {
		log.Printf("can't get hash from current template: %v", err)
		return false
	}
	if prevTemplateHash != *createHashResult.TemplateHash {
		log.Printf("template hash is different from previous deployment")
		return false
	}
	...
	if !goodCast {
		log.Printf("unexpected type found on previous deployment parameters. Ignoring deployment")
		return false
	}
	...
	for prevParamKey, prevParamData := range prevParams {
		...
		if prevParamInfo["value"] != currentParamInfo["value"] {
			log.Printf("parameter %s has a new value", prevParamKey)
			return false
		}
	}
...
func (p *BicepProvider) Deploy(ctx context.Context) (*DeployResult, error) {
	...
	if p.alphaFeatureManager.IsEnabled(alpha.BicepCache) {
		p.console.ShowSpinner(ctx, "Looking for last deployment.", input.Step)
		prevDeploymentResult, err := p.prevDeploymentResult(ctx, bicepDeploymentData.Target)
		if err != nil {
			log.Println("error: unable to get previous deployment. Ignoring any previous deployment.")
		}
