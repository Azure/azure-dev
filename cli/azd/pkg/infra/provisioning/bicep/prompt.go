...
func (p *BicepProvider) promptForParameter(
	...
	if paramType == ParameterTypeString && azdMetadata.Type != nil && *azdMetadata.Type == "location" {
		...
		value = location
	} else if paramType == ParameterTypeString && azdMetadata.Type != nil && *azdMetadata.Type == "autoOrManualGeneration" {
		// default is auto generation. manualUserInput is set to true only when requested by user
		var manualUserInput bool
		if azdMetadata.NoManual == nil || (azdMetadata.NoManual != nil && !*azdMetadata.NoManual) {
			// manual input can only be enabled by asking user for it.
			// If metadata defines NoManual as true, then user is not asked and auto-gen is used directly.
		...
		if manualUserInput {
			...
			if azdMetadata.AutoGenerateConfig == nil {
				return nil, fmt.Errorf("auto generation metadata config is missing for parameter '%s'", key)
			}
