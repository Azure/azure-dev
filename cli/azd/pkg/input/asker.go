...
func init() {
	...
	{{- else }}
		{{- "  "}}{{- color "cyan"}}[Use arrows to move, space to select, enter to continue, type to filter{{- if and .Help (not .ShowHelp)}}, {{ .Config.HelpInput }} for more help{{end}}]{{color "reset"}}
	  {{- "\n"}}
