...
func (sc *silentConsole) ShowSpinner(ctx context.Context, title string, format SpinnerUxType) {
	log.Printf("request to show spinner on silent console with message: %s", title)
}

func (sc *silentConsole) StopSpinner(ctx context.Context, lastMessage string, format SpinnerUxType) {
	log.Printf("request to stop spinner on silent console with message: %s", lastMessage)
}

// Prompts the user for a single value
func (sc *silentConsole) Prompt(ctx context.Context, options ConsoleOptions) (string, error) {
	log.Panic("Can't call Prompt from a silent console.")
	return "", nil
}

// Prompts the user to select from a set of values
func (sc *silentConsole) Select(ctx context.Context, options ConsoleOptions) (int, error) {
	log.Panic("Can't call Select from a silent console.")
	return 0, nil
}

// Prompts the user to confirm an operation
func (sc *silentConsole) Confirm(ctx context.Context, options ConsoleOptions) (bool, error) {
...
func (sc *silentConsole) Handles() ConsoleHandles {
	return ConsoleHandles{}
}

// Creates a silent console
func NewSilentConsole() Console {
