...
// FeatureManager provides operations for handling features within the application which are in option-out mode.
type FeatureManager struct {
	configManager   config.UserConfigManager
	userConfigCache config.Config
	// used for mocking option-out features on testing
	optionOutFeaturesResolver func() []Feature
	withSync                  *sync.Once
}

// NewFeaturesManager creates the option-out features manager from the user configuration
...
// ListFeatures pulls the list of features in option-out mode available within the application and displays its current state
// which is `on` or `off`.
func (m *FeatureManager) ListFeatures() (map[string]Feature, error) {
	result := make(map[string]Feature)

	optionOutFeatures := allFeatures
	if m.optionOutFeaturesResolver != nil {
		optionOutFeatures = m.optionOutFeaturesResolver()
	}

	for _, optionOutputFeature := range optionOutFeatures {
		// cast is safe here from string to OptionOutFeatureId
		status := disabledText
		if m.IsEnabled(FeatureId(optionOutputFeature.Id)) {
			status = enabledText
		}

		result[optionOutputFeature.Id] = Feature{
			Id:          optionOutputFeature.Id,
			Description: optionOutputFeature.Description,
			Status:      status,
		}
	}
...
// IsEnabled search and find out if the OptionOutFeatureId is currently enabled
func (m *FeatureManager) IsEnabled(featureId FeatureId) bool {
	// guard from using the optionOutFeatureManager from multiple routines. Only the first one will create the cache.
	...
	envVarNames := []string{
		fmt.Sprintf("AZD_OPTION_OUT_ENABLE_%s", strings.ToUpper(string(featureId))),
		fmt.Sprintf("AZD_OPTION_OUT_ENABLE_%s", strings.ReplaceAll(strings.ToUpper(string(featureId)), ".", "_")),
	}

	// For testing, and in CI, allow enabling option-out features via the environment.
	...
	if foundEnvVar {
		...
		enabled = true
		tracing.AppendUsageAttributeUnique(fields.OptionOutFeaturesKey.String(string(AllId)))
	...
	if enabled {
		tracing.AppendUsageAttributeUnique(fields.OptionOutFeaturesKey.String(string(featureId)))
	}
