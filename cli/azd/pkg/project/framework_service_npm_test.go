...
func Test_NpmProject_Package_EmptySource(t *testing.T) {
	tempDir := t.TempDir()
	ostest.Chdir(t, tempDir)

	// runArgs is a variable of type exec.RunArgs that is used to store the arguments
	// required to execute a command. This variable is typically used in test cases
	// to simulate the execution of a command within the npm framework service tests.
	var runArgs exec.RunArgs
	...
	_, err = logProgress(t, func(progress *async.Progress[ServiceProgress]) (*ServicePackageResult, error) {
		return npmProject.Package(
			*mockContext.Context,
			serviceConfig,
			&ServiceBuildResult{
				BuildOutputPath: filepath.Join(serviceConfig.Path(), "empty"),
			},
			progress,
		)
	})
	...
	require.Error(t, err)
	require.Contains(t, err.Error(), "package source")
}

func Test_NpmProject_Package_ValidSource(t *testing.T) {
	tempDir := t.TempDir()
	ostest.Chdir(t, tempDir)
	...
	err = os.WriteFile(filepath.Join(serviceConfig.Path(), "package.json"), nil, osutil.PermissionFile)
	require.NoError(t, err)
	err = os.MkdirAll(filepath.Join(serviceConfig.Path(), "dist"), osutil.PermissionDirectory)
	require.NoError(t, err)
	err = os.WriteFile(filepath.Join(serviceConfig.Path(), "dist", "index.js"), []byte("console.log('Hello World');"), osutil.PermissionFile)
	require.NoError(t, err)
