...
func (pp *pythonProject) Restore(
	...
	pp.publisher.Send(ctx, progress.NewMessage("Checking for Python virtual environment"))
	...
	if err != nil {
		if os.IsNotExist(err) {
			pp.publisher.Send(ctx, progress.NewMessage("Creating Python virtual environment"))
			...
			return nil, fmt.Errorf(
				"python virtual environment for project '%s' is not accessible: %w",
				serviceConfig.Path(),
				err,
			)
		}
	...
	pp.publisher.Send(ctx, progress.NewMessage("Installing Python PIP dependencies"))
...
func (pp *pythonProject) Package(
	...
	pp.publisher.Send(ctx, progress.NewMessage("Copying deployment package"))
	if err := buildForZip(
		packageSource,
		packageDest,
		buildForZipOptions{
