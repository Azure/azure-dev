...
func Test_pythonProject_RequiredExternalTools_Coverage3(t *testing.T) {
	...
	tools := p.(FrameworkService).RequiredExternalTools(t.Context(), &ServiceConfig{})
...
func Test_nodeProject_Requirements_Coverage3(t *testing.T) {
	p := NewNodeProject(node.NewCli(exec.NewCommandRunner(nil)), environment.NewWithValues("test", nil), exec.NewCommandRunner(nil))
	reqs := p.(FrameworkService).Requirements()
...
func Test_mavenProject_Requirements_Coverage3(t *testing.T) {
	p := NewMavenProject(environment.NewWithValues("test", nil), maven.NewCli(exec.NewCommandRunner(nil)), javac.NewCli(exec.NewCommandRunner(nil)))
	reqs := p.(FrameworkService).Requirements()
	assert.False(t, reqs.Package.RequireRestore)
	assert.False(t, reqs.Package.RequireBuild)
}
