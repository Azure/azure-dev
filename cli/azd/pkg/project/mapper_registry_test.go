...
func TestLayerDependsOnMapping(t *testing.T) {
	...
	// prove that that the protoLayer is a copy, not a reference
	protoBufModel.DependsOn[0] = "changed"
	require.Equal(t, []string{"foundation"}, projectLayer.DependsOn, "DependsOn is unchanged")

	// and now convert from protobuf to our internal models
	protoBufModel.DependsOn[0] = "foundation"
	var roundTrip *LayerConfig
	require.NoError(t, mapper.Convert(protoBufModel, &roundTrip))
	require.Equal(t, projectLayer.DependsOn, roundTrip.DependsOn)

	protoBufModel.DependsOn[0] = "changed-again"
	require.Equal(t, []string{"foundation"}, roundTrip.DependsOn)
}
