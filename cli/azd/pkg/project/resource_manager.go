...
func (rm *resourceManager) GetServiceResource(
	...
	if err != nil {
		return azapi.AzCliResource{}, fmt.Errorf("expanding name: %w", err)
	}

	resources, err := rm.GetServiceResources(ctx, subscriptionId, resourceGroupName, serviceConfig)
	if err != nil {
		return azapi.AzCliResource{}, fmt.Errorf("getting service resource: %w", err)
	}
	...
	if expandedResourceName == "" { // A tag search was performed
		if len(resources) == 0 {
			...
			return azapi.AzCliResource{}, azureutil.ResourceNotFound(err)
		}

		if len(resources) != 1 {
			return azapi.AzCliResource{}, fmt.Errorf(
				//nolint:lll
...
func (rm *resourceManager) resolveServiceResource(
	...
	if err != nil {
		return azapi.AzCliResource{}, err
	}
