...
func Test_CLI_Init_AsksForSubscriptionIdAndCreatesEnvAndProjectFile(t *testing.T) {
	...
	proj, err := project.LoadProjectConfig(filepath.Join(dir, azd_context.ProjectFileName), &environment.Environment{})
	require.NoError(t, err)
...
func Internal_Test_CLI_ResourceGroupsName(t *testing.T, envName string, rgName string, includeEnvNameTag bool, createResources bool, createMultipleResourceGroups bool) {
	...
	envFilePath := filepath.Join(dir, azd_context.EnvironmentDirectoryName, envName, ".env")
...
func Test_CLI_InfraCreateAndDeleteWebApp(t *testing.T) {
	...
	env, err := godotenv.Read(filepath.Join(dir, azd_context.EnvironmentDirectoryName, envName, ".env"))
	require.NoError(t, err)
	...
	err = godotenv.Write(env, filepath.Join(dir, azd_context.EnvironmentDirectoryName, envName, ".env"))
	require.NoError(t, err)
...
func getTestEnvPath(dir string, envName string) string {
	return filepath.Join(dir, azd_context.EnvironmentDirectoryName, envName, ".env")
}
