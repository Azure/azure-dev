...
- Extensions published to the official registry can report **usage events** via
  ...
  `cli/azd/extensions/telemetry/fields.go`; a source-only repository test blocks
  undeclared or dynamically keyed attributes before release. Only extensions
  whose configured source matches the verified official `azd` registry name,
  type, and normalized URL are recorded — see
