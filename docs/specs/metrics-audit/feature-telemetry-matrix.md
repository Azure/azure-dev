...
|-----------|---------|--------|----------------|-------|
| **Execution environment detection** | Every command with telemetry enabled | (none — resource attribute) | `execution.environment` | Detects local/hosted environments, CI providers, and AI coding agents. Antigravity is reported as its own fixed value rather than Gemini. |
