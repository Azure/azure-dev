...
| Extensions (dynamic) | `extension.id`, `extension.version`, `extension.version.from`, `extension.version.to`, `extension.source.category`, `extension.source.kind`, `extension.source.category.from`, `extension.source.category.to`, `extension.installed.source.category`, `extension.dependency_of`, `extension.dependency_update_count`, `extension.update.outcome`, `extension.update.duration_ms`, `extension.grpc.legacy_call_count` + trace-context propagation to child process | Covers `ext.run`, `ext.install`, `ext.update`, `ext.promote`, source registration, installed inventory, and temporary legacy gRPC bridge use without emitting source names or locations |
