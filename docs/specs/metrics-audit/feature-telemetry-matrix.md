...
| Provision layers | `provision.layer.*` | `provision`, `up` | Layer-graph format and shape: `is_v2`, `count`, `max_parallel`, `safe_fallback_count`, `explicit_dependson_count` (one bool and four integer measurements; no duration or outcome) |
...
| **Provision layers** | `provision`, `up` | (none — enriches the `provision` span) | `provision.layer.is_v2`, `provision.layer.count`, `provision.layer.max_parallel`, `provision.layer.safe_fallback_count`, `provision.layer.explicit_dependson_count` | Format, count, and explicit dependencies are always emitted; topology measurements require successful analysis or a trivial zero/one-entry topology. No per-layer duration or outcome attribute is emitted |
