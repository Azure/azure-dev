...
| **Azure AI Dataset publish** | `azure.ai.dataset` writes a dataset version (create or update) | `ext.usage` with `extension.event=dataset.published` | `ext.operation` (`create`, `update`, or `unknown`) | Reported once after the publish succeeds; the verb is normalized to the closed set and carries no dataset name or content |
| **Azure AI Evaluations init** | `azure.ai.evaluations` `init` scaffolds an evaluation config | `ext.usage` with `extension.event=init.completed` | `ext.source` (`traces`, `dataset`, or `unknown`) | Emitted once after the scaffold is written to disk; identifies the rows the eval will grade, not any dataset name or content |
