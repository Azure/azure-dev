...
| **Multi-layer provision** | `provision` (when `infra.layers[]` or top-level `layers[].infra[]` is configured in `azure.yaml`) | (none — enriches the `provision` span) | `provision.layer.is_v2`, `provision.layer.count`, `provision.layer.max_parallel`, `provision.layer.safe_fallback_count`, `provision.layer.explicit_dependson_count` | One format boolean and four integer measurements are emitted from `internal/cmd/provision_graph.go`; no per-layer duration or outcome attribute is emitted |
