...
| **Extension invocation errors** | An extension command returns an error | `ext.run` | `extension.id`, `extension.version`, `extension.event`, `error.chain.types`, `error.mapper.source.type`, `error.mapper.destination.type` | `MapError` attributes the failed span to the executing extension and preserves bounded error-chain and mapper diagnostics; `extension.installed` remains an inventory signal |
