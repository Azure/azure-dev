...
| Provision validation outcome | `validation.provision.outcome` (+ peer fields) | `provision` | Distinguishes passed / warnings-accepted / canceled local validation |
...
| **Provision validation** | `provision` (prior to ARM deploy) | `validation.provision` | `validation.provision.outcome`, plus 4 peer fields covering warnings/errors counts and cancel reason | Local-only validation; outcome captures passed / warnings-accepted / canceled |
