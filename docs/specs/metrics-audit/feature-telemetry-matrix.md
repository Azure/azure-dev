...
| **Extension invocation errors** | An extension command or lifecycle hook returns an error | `ext.run` for commands; enclosing `cmd.*` for lifecycle hooks | `extension.id`, `extension.version`, `extension.event` (lifecycle hooks), `error.chain.types`, `error.mapper.source.type`, `error.mapper.destination.type`, `error.tool.name` | `MapError` attributes the failed span to the executing extension and preserves bounded error-chain, mapper, and tool diagnostics; failed commands do not set `extension.event`, and `extension.installed` remains an inventory signal |
