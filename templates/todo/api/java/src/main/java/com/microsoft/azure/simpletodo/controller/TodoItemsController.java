...
public class TodoItemsController implements ItemsApi {
	...
	public ResponseEntity<Void> deleteItemById(String listId, String itemId) {
		return todoItemRepository.findTodoItemByListIdAndId(listId, itemId)
				...
				.map(i -> ResponseEntity.noContent().<Void>build()).orElse(ResponseEntity.notFound().build());
	}

	public ResponseEntity<TodoItem> getItemById(String listId, String itemId) {
		return todoItemRepository.findTodoItemByListIdAndId(listId, itemId).map(ResponseEntity::ok)
	...
	public ResponseEntity<TodoItem> updateItemById(String listId, String itemId, TodoItem todoItem) {
		...
		todoItem.setListId(listId);
		return todoItemRepository.findTodoItemByListIdAndId(listId, itemId).map(t -> todoItemRepository.save(todoItem))
	...
	public ResponseEntity<List<TodoItem>> getItemsByListIdAndState(String listId, TodoState state, BigDecimal top,
		...
		return todoListRepository
				.findById(listId).map(l -> todoItemRepository.findTodoItemsByTodoListAndState(l.getId(), state.name(),
						skip.intValue(), top.intValue()))
				.map(ResponseEntity::ok).orElseGet(() -> ResponseEntity.notFound().build());
	}
