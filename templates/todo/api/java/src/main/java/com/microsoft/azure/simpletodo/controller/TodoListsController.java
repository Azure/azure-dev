...
public class TodoListsController implements ListsApi {
	...
	public ResponseEntity<Void> deleteListById(String listId) {
		return todoListRepository.findById(listId).map(l -> todoListRepository.deleteTodoListById(l.getId()))
				.map(l -> ResponseEntity.noContent().<Void>build()).orElseGet(() -> ResponseEntity.notFound().build());
	}

	public ResponseEntity<TodoList> getListById(String listId) {
		return todoListRepository.findById(listId).map(ResponseEntity::ok)
	...
	public ResponseEntity<TodoList> updateListById(String listId, @NotNull TodoList todoList) {
		...
		return todoListRepository.findById(listId).map(t -> ResponseEntity.ok(todoListRepository.save(todoList)))
				.orElseGet(() -> ResponseEntity.notFound().build());
	}
